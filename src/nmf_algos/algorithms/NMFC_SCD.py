"""SCD with missing entries. Python version implemented similarly as R package NNLM. """
import os
import time
import copy
import numpy as np
    ...
    while (inner_iter < inner_max_iter) and (rel_err > inner_rel_tol):
        ...
        for k in range(r):
            ...
            if tmp!=Hj[k]:
                ...
                continue
            etmp = 2*np.abs(Hj[k]-tmp) / (tmp + Hj[k] + TINY_NUM)
            rel_err = max(rel_err, np.max(etmp))
            Hj[k] = tmp
        inner_iter +=1
...
#         #print("j:", j, "error:", project_error(A, W, H.T, known_mask)[0])
#     return H, rel_err
...
def scd_ls_update_column_parallel(args):
    ...
    mu = W_known.T@Aj_known
    mu = WtW @ Hj - mu
    Hj, mu, rel_err = scd_ls_update_column(Hj, mu, WtW, inner_rel_tol, inner_max_iter)
    return Hj
...
def scd_ls_update_factor(A, W, H, known_mask, inner_rel_tol, inner_max_iter, beta):
    ...
    with Pool() as pool:
        ...
        column_idx = 0
        for result in pool.imap(scd_ls_update_column_parallel, [(A[:,j],  W, H[:, j], known_mask[:, j], beta,  inner_rel_tol, inner_max_iter) for j in range(n)]):
...
class NMFC_SCD(NMFBase):
    ...
    def factor_init(self, params):
        if "known_mask" not in params:
            raise NotImplementedError("Please provide a known_mask to separate out missing entries in X!")
    ...
    def one_iter(self, A, W, H, previous_err):
        ...
        H, _ = scd_ls_update_factor(A, W, H, bool_known_mask, self.inner_rel_tol, self.inner_max_iter, self.beta)
        obj, _ = project_error(A, W, H.T, self.known_mask)
        print("factor one side", obj)
        Wt, _ = scd_ls_update_factor(A.T, H.T, W.T, bool_known_mask.T, self.inner_rel_tol, self.inner_max_iter, self.beta)
        W = Wt.T
        obj, _ = project_error(A, W, H.T, self.known_mask)
        print("factor another side", obj)
        current_errr = 0.5*obj
        rel_err = 2*(previous_err - current_errr )/(previous_err + current_errr + TINY_NUM)
    ...
    def core_run(self, f_continue_cond, verbose=True, save_time_error=True):
        ...
        while continue_cond and (not exit_cond):
            ...
            if (n_iter % 20 == 0) or exit_cond:
                ...
                if save_time_error:
                    self.tracker(cur_iter_left_factor, cur_iter_right_factor, self.iter_save_dir, n_iter, {"iter_time": cur_time, "iter_error":obj})
                else:
                    self.tracker(cur_iter_left_factor, cur_iter_right_factor, self.iter_save_dir, n_iter)
...
def test():
    ...
    for r in [5, 10, 15, 20, 25]:
        params = {"X": org_data_mat, "dataset_name": "Movielens", "r": r, "rerun_times": 2}
        params["known_mask"] = (org_data_mat > 0).astype(int)
        nmfc_scd = NMFC_SCD(method_name=method_name, params=params)
        nmfc_scd.basic_run()
